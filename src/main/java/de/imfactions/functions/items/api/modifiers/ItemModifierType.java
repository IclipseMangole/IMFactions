...
public static final ItemModifierType<Double> LIFESTEAL = new ItemModifierType("lifesteal", "Lifesteal", ChatColor.of("#C20000"), Double.class, true);
   ...
   public ItemModifierType(String name, String displayName, ChatColor chatColor, Class<?> type, boolean special) {     this.name = name;     this.displayName = displayName;     this.chatColor = chatColor;     this.type = type;     if (special) {       if (type == Double.class) {         this.roman = false;         this.percent = true;       } else if (type == Integer.class) {         this.roman = true;         this.percent = false;
       } else {         this.roman = false;         this.percent = false;
       }
     } else {       this.roman = false;       this.percent = false;
     }     itemModifierTypes.put(name, this);
   }

   public ItemModifierType(String name, String displayName, ChatColor chatColor, Class<?> type) {     this(name, displayName, chatColor, type, false);
   }
   ...
   public String toString(T value) {     String valueString = value.toString();     if (this.roman) {       valueString = RomanNumber.toRoman(((Integer)value).intValue());
     }     if (this.percent) {       DecimalFormat format = new DecimalFormat("#.#");       valueString = format.format(((Double)value).doubleValue() * 100.0D) + "%";
     }     return this.chatColor + this.displayName + " " + valueString;
   }


   public String toString() {     return "ItemModifierType{key=" + this.name + ",type=" + this.type + '}';
   }

   public static ItemModifierType getByName(String name) {     return itemModifierTypes.get(name);
   }

   public static ItemModifierType<?>[] values() {     return (ItemModifierType<?>[])itemModifierTypes.values().toArray();
   }
 }
