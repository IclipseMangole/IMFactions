...
public class UserSettingsTable {
    public void UserSettingsTable() {
        IMFactions.getInstance().getData().getMySQL().update("CREATE TABLE IF NOT EXISTS usersettings (id MEDIUMINT NOT NULL AUTO_INCREMENT, uuid VARCHAR(60), `key` VARCHAR(256), `value` VARCHAR(256), PRIMARY KEY (id))");
    }

    public void createUserSetting(UUID uuid, String key, String value) {
        if (!isSettingExists(uuid, key)) {
            IMFactions.getInstance().getData().getMySQL().update("INSERT INTO usersettings (uuid, `key`, `value`) VALUES ('" + uuid + "', '" + key + "', '" + value + "')");
        }
    }
    ...
    public void deleteUserSetting(UUID uuid, String key) {
        IMFactions.getInstance().getData().getMySQL().update("DELETE usersettings WHERE uuid = '" + uuid + "' AND `key` = '" + key + "'");
    }

    public int getId(UUID uuid, String key) {
        try {
            ResultSet rs = IMFactions.getInstance().getData().getMySQL().querry("SELECT id FROM usersettings WHERE uuid = '" + uuid + "' AND `key` = '" + key + "'");
            while (rs.next()) {
    ...
    public String getString(UUID uuid, String key) {
        try {
            ResultSet rs = IMFactions.getInstance().getData().getMySQL().querry("SELECT `value` FROM usersettings WHERE uuid = '" + uuid + "' AND `key` = '" + key + "'");
            while (rs.next()) {
    ...
    public void setString(UUID uuid, String key, String value) {
        IMFactions.getInstance().getData().getMySQL().update("UPDATE usersettings SET `value` = '" + value + "' WHERE uuid = '" + uuid + "' AND `key` = '" + key + "'");
    }
